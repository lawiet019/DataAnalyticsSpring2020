...
library(ggplot2)
library(e1071)
# we will seperate the Species based on the color and plot with
#Petal.Legth Vs Petal.Width
# We can clearly see the seperation of Setosa however, there
#is an overlappings in Versicolor and Virginica.
# Now we plot using the qplot() fuction, X=Petal.Length, Y =
#Petal.Width using the color seperation respect to Species.
qplot(Petal.Length, Petal.Width, data=iris, color = Species)

# Now we can use the built in svm() function that
#comes in the e1071 library
...
# Using the summary command to see the summary
#of our first model, we pass the svm_model1 to the
# summary() function
summary(svm_model1)
...
# Prediction using the model (svm_model1) we created on the
#iris dataset.
pred1 <- predict(svm_model1, iris)
# creating a table using the predicted one and the actual iris
#dataset
...
# Using the summary command to see the summary of our second model,
#we pass the svm_model2 to the
# summary() function
summary(svm_model2)
# Using the plot() function and our second model
#which is svm_model2, we can plot the results
# here the axes are Petal.Width Vs Petal.Length
plot(svm_model2, data = iris,
     Petal.Width~Petal.Length, slice = list(Sepal.Width = 3,
                                            Sepal.Length = 4))
# Prediction using the second model (svm_model2) we created
#on the iris dataset.
